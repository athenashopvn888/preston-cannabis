...
export type Menu = { products: Product[]; state: "connected" | "unconfigured" | "unavailable"; fetchedAt: string; stockDate: string; sourceIdentity: string; source: "CHC01" };
export const getMenu = cache(async (): Promise<Menu> => {
  const empty: Menu = { products: [], state: "unconfigured", fetchedAt: "", stockDate: "", sourceIdentity: "unverified", source: "CHC01" };
  if (!process.env.APPS_SCRIPT_URL) return empty;
  try {
    const code = process.env.MENU_STORE_CODE || "CHC01";
    if (code !== "CHC01") return { ...empty, state: "unavailable" };
