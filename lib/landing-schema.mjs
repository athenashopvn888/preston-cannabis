...
export function businessGraph(site, faqs = /** @type {{question:string,answer:string}[]} */ ([])) {
  ...
  // Published weekly hours (overnight closes stay on the same dayOfWeek).
  if (Array.isArray(HOURS_STATUS.weekly) && HOURS_STATUS.weekly.length) {
    store.openingHoursSpecification = HOURS_STATUS.weekly.map((row) => ({
      "@type": "OpeningHoursSpecification",
      dayOfWeek: row.dayOfWeek,
      opens: row.opens,
      closes: row.closes,
    }));
  } else if (HOURS_STATUS.is24h) {
